...
set_ss_reboot_job(){
	if [[ "${ss_reboot_check}" == "0" ]]; then
		...
		echo_date 【菠萝云】：设置每天${ss_basic_time_hour}时${ss_basic_time_min}分重启插件...
		cru a ss_reboot ${ss_basic_time_min} ${ss_basic_time_hour}" * * * /bin/sh /koolshare/ss/ssconfig.sh restart"
	elif [[ "${ss_reboot_check}" == "2" ]]; then
		echo_date 【菠萝云】：设置每周${ss_basic_week}的${ss_basic_time_hour}时${ss_basic_time_min}分重启插件...
		...
		echo_date 【菠萝云】：设置每月${ss_basic_day}日${ss_basic_time_hour}时${ss_basic_time_min}分重启插件...
		...
		echo_date 【菠萝云】：设置每天${check_custom_time}时的${ss_basic_time_min}分重启插件...
...
set_ss_trigger_job(){
	if [ "$ss_basic_tri_reboot_time" == "0" ];then
		...
		echo_date 菠萝云插件触发重启功能的日志将显示再系统日志内。
	...
	if [ -f "/tmp/ss_host.conf" ];then
		...
		if [ -n "$HOST" ] && [ -n "$OLD_IP" ];then
			...
			if [ "$?" == "0" ];then
				...
				logger 【菠萝云插件触发重启功能】：SS服务器域名解析失败！
			fi

			if [ "$OLD_IP"x == "$NEW_IP"x ];then
				logger 【菠萝云插件触发重启功能】：服务器ip地址未发生变化，不进行任何操作！
			else
				logger 【菠萝云插件触发重启功能】：服务器ip地址发生变化，旧ip：【"$OLD_IP"】，新ip：【"$NEW_IP"】
				#写入新的解析文件
				echo "address=/$HOST/$NEW_IP" > /tmp/ss_host.conf
				if [ "$ss_basic_tri_reboot_policy" == "1" ];then
					logger 【菠萝云插件触发重启功能】：重启整个插件，以应用新的ip
					sh /koolshare/ss/ssconfig.sh restart
				else
					logger 【菠萝云插件触发重启功能】：重启dnsmasq，以应用新的ip
					[ -L "/jffs/configs/dnsmasq.d/ss_host.conf" ] && rm -rf /jffs/configs/dnsmasq.d/ss_host.conf
					service restart_dnsmasq >/dev/null 2>&1
				fi
			fi
		else
			logger 【菠萝云插件触发重启功能】：未找到你当前节点的服务器地址，可能插件提交时未正确解析！
			logger 【菠萝云插件触发重启功能】：请尝试直接使用ip地址作为服务器地址！
		fi
	else
		if [ -n "$ss_basic_server_ip" ];then
			IFIP=`echo $ss_basic_server_ip|grep -E "([0-9]{1,3}[\.]){3}[0-9]{1,3}|:"`
			logger 【菠萝云插件触发重启功能】：当前节点的服务器地址已经是IP格式！不进行任何操作！
		else
			logger 【菠萝云插件触发重启功能】：未找到你当前节点的服务器地址，可能已是IP格式！不进行任何操作！
		fi
	fi
}
# -------------------
